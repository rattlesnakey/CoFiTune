...
class DeepSpeedZeroOptimizer_Stage3(ZeROOptimizer):
    ...
    def set_mask_to_grad_for_param(self, param, mask=None, args=None) -> Tensor:
        if not param.requires_grad:
            return None
        ...
        if self.offload_optimizer:
            group_idx, dest_offset, num_elements = self.grad_position[self.get_param_id(param)]
            origin_fp32_grad = self.fp32_partitioned_groups_flat[group_idx].grad.narrow(0, dest_offset, num_elements) 

            mask = mask.to(device=origin_fp32_grad.device, dtype=origin_fp32_grad.dtype)


            repeat_num = origin_fp32_grad.size(0) // mask.size(0)
            rest_num = origin_fp32_grad.size(0) % mask.size(0)
            if rest_num != 0:
                mask_list = [mask]*repeat_num + [mask[:rest_num]]
            else:
                mask_list = [mask]*repeat_num
    
            
            self.fp32_partitioned_groups_flat[group_idx].grad.narrow(0, dest_offset, num_elements).copy_(origin_fp32_grad * self.flatten(mask_list))
        
        else:
            assert origin_fp32_grad.size(0) % mask.size(0) == 0, 'cannot even spilt param!!'
            origin_fp32_grad = self.__param_id_to_grad_partition[param.ds_id]
            mask = mask.to(device=origin_fp32_grad.device, dtype=origin_fp32_grad.dtype)
            repeat_num = origin_fp32_grad.size(0) // mask.size(0)
            rest_num = origin_fp32_grad.size(0) % mask.size(0)
            ...
            if rest_num != 0:
                ...
                mask_list = [mask]*repeat_num
            self.__param_id_to_grad_partition[param.ds_id].copy_(origin_fp32_grad * self.flatten(mask_list))
